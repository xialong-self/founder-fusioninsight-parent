 ...
 * @date 2020-09-24
 */
public class jwzh_jcgl_UpdateDpxx {
    /**
     * 每日中午两点全量跟新三表，以及更新dpxx所有为空qfjcdm_mc
     */
    public static void main(String[] args) {
        FusioninsightConfig.initArgs(args);
        new jwzh_jcgl_UpdateDpxx().run();
    ...
    public void run() {
        ...
        String beforetime= DateTimeUtils.DateToString(DateTimeUtils.addDays(DateTimeUtils.StringToDate(time, DateTimeUtils.YYYY_MM_DD_HH_MM_SS), -30), DateTimeUtils.YYYY_MM_DD);
        ...
        Dataset<Row> dpxx_yx= EsSparkSQL.esDF(sparkSession,"jwzh_jcgl.tb_st_dpxx/_doc")
                .filter("qfjcdm_mc is null ")
                .selectExpr("id","qfjcdm","ddjcdm");
        Dataset<Row> dpxx_all=DataChange(dpxx_yx,zdxxdata_y,gtxxdata_y).filter("qfjcdm_mc is not null");
        ...
        Dataset<Row>  esdata=EsSparkSQL.esDF(sparkSession,"jwzh_jcgl.tb_st_dpxx/_doc")
                .filter("xtLrsj>='"+beforetime+"' and qfjcdm_mc is not null ")
